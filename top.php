...
// Local development: mock REMOTE_USER from .env so the app works without CAS.
// Set APP_LOCAL_DEV_USER in your .env to your UVM NetID (e.g. aperkel).
// Leave it empty or unset in production.
if (empty($_SERVER['REMOTE_USER']) && !empty($_ENV['APP_LOCAL_DEV_USER'])) {
    $_SERVER['REMOTE_USER'] = $_ENV['APP_LOCAL_DEV_USER'];
}

// Enforce allowed user list (DB or .env depending on APP_USE_DB_USERS).
requireUser();
?>
<!DOCTYPE HTML>
<html lang="en">
...
<head>
    ...
    <meta name="apple-mobile-web-app-title" content="77 N Union" />
