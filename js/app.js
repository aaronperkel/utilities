/**
 * 77 N Union Utilities - Frontend JavaScript
...
/**
 * Auto-calculate total (amount + processing fee) and per-person cost.
 * Reads the processing fee from the selected bill-type option's data-fee attribute
 * and people count from #add-bill-form's data-people-count attribute.
 */
function initAutoCalcCost() {
    const form       = document.getElementById('add-bill-form');
    const amountInput = document.getElementById('amount');
    const totalInput  = document.getElementById('total-display');
    const costInput   = document.getElementById('cost-display');
    const typeSelect  = document.getElementById('item');
    const feeHint     = document.getElementById('fee-hint');

    if (!form || !amountInput || !totalInput || !costInput || !typeSelect) return;

    const peopleCount = parseInt(form.dataset.peopleCount) || 3;
    ...
    function recalc() {
        ...
        totalInput.value = base > 0 ? total.toFixed(2) : '';
        costInput.value  = base > 0
            ? (Math.round((total / peopleCount) * 100) / 100).toFixed(2)
            : '';

        if (feeHint) {
            feeHint.textContent = fee > 0 ? `(+$${fee.toFixed(2)} fee)` : '';
        }
    }
